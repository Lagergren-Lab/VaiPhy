...
for NUM_POS in 300 1000 
do
	for DATA_SEED in 1 2 3
	do
		for NUM_TAXA in 10 20 40 
		do
			...
			do
				...
				do
					RESULT_DIR=${RES_DIR}
					EXP_NAME="data_seed_${DATA_SEED}_taxa_${NUM_TAXA}_pos_${NUM_POS}"
...
#SBATCH -n 1
#SBATCH -t 6:00:00
#SBATCH -J prep_${EXP_NAME}
#SBATCH -e ${RESULT_DIR}/${EXP_NAME}/prep_${VAIPHY_NAME}_err.txt
#SBATCH -o ${RESULT_DIR}/${EXP_NAME}/prep_${VAIPHY_NAME}_out.txt
...
python post_analysis_prep.py --dataset ${EXP_NAME} --vaiphy_seed ${VAIPHY_SEED} --n_particles ${NUM_PARTICLES} --ng_stepsize ${NG_STEPSIZE} --max_iter ${MAX_ITER} --init_strategy ${INIT_STRATEGY} --samp_strategy ${SAMP_STRATEGY} --branch_strategy ${BRANCH_STRATEGY} 
wait
