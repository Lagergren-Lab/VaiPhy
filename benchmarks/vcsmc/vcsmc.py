...
class VCSMC:
    ...
    def __init__(self, datadict, K, args=None, seed=0):
        tf.set_random_seed(seed)  # seed added by us for reproducibility
        np.random.seed(seed)      # seed added by us for reproducibility
        self.args = args
    ...
    def compute_forest_posterior(self, data_KxXxSxA, leafnode_num_record, r):
        ...
        return forest_loglik + forest_logprior  # p(y,t | b)
    ...
    def train(self, epochs=100, batch_size=128, learning_rate=0.001, memory_optimization='on'):
        ...
        print("Directory is: \t", save_dir, flush=True)
        ...
        for i in tqdm(range(epochs)):
            ...
            print('RB param:\n', rb_param)
            if i+1 == epochs:
                print('Mean LL_R: ', np.mean(log_lik_R), '\tStd LL_R: ', np.std(log_lik_R),
                      '\tMax LL_R: ', np.max(log_lik_R))
        ...
        print('Mean Best_LL_R: ', np.mean(best_log_lik), '\tStd Best_LL_R: ', np.std(best_log_lik),
              '\tMax Best_LL_R: ', np.max(best_log_lik))
